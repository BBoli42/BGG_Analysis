...
def main():
    ...
    make_folder()
    missed_count = 0
    for i in range(1,config.max_game):
        current_time = datetime.datetime.now()
        seconds_passed = (current_time-start_time).total_seconds()
        total_seconds = (config.max_game-1) / float(i+1-1) * seconds_passed
        print(f'{round((i-1)/(config.max_game-1) * 100, 2)}% done, {total_seconds-seconds_passed} seconds remain')
